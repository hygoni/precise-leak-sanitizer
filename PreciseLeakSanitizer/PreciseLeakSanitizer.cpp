...
void PreciseLeakSanVisitor::visitCallNew(CallInst &I) {
  return;
}

void PreciseLeakSanVisitor::visitCallArrTyNew(CallInst &I) {
  return;
}
...
void PreciseLeakSanVisitor::visitCallMemset(CallInst &I) {
  return;
}
...
void PreciseLeakSanVisitor::visitCallMemmove(CallInst &I) {
  return;
}

void PreciseLeakSanVisitor::visitCallBzero(CallInst &I) {
  return;
}

bool PreciseLeakSanitizer::initializeModule(Module &M) {
  return false;
}
...
PreservedAnalyses PreciseLeakSanitizerPass::run(Module &M, ModuleAnalysisManager &) {
  return (PreciseLeakSanitizer().run(M) ? PreservedAnalyses::none() : PreservedAnalyses::all());
}

llvm::PassPluginLibraryInfo getPreciseLeakSanitizerPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "precise-leak-sanitizer", LLVM_VERSION_STRING,
    [](PassBuilder &PB) {
