...
// Standalone PLSan RTL code common to POSIX-like systems.
//
//===---------------------------------------------------------------------===//
...
ThreadContext::ThreadContext(int tid) : ThreadContextPlsanBase(tid) {}
...
void ThreadContext::OnStarted(void *arg) {
  ThreadContextPlsanBase::OnStarted(arg);
...
void ThreadStart(u32 tid, tid_t os_id, ThreadType thread_type) {
  ...
  args.dtls = DTLS_Get();
  ThreadContextPlsanBase::ThreadStart(tid, os_id, thread_type, &args);
}
...
bool GetThreadRangesLocked(tid_t os_id, uptr *stack_begin, uptr *stack_end,
  ...
  ThreadContext *context = static_cast<ThreadContext *>(
      GetPlsanThreadRegistryLocked()->FindThreadContextByOsIDLocked(os_id));
  if (!context)
    return false;
